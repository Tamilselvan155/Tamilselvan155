# Tamilselvan C

<p align="center">
  <img src="https://readme-typing-svg.demolab.com?font=Inter&weight=600&size=22&pause=1000&center=true&vCenter=true&width=600&lines=Full+Stack+Developer;MERN+Stack+%7C+Java+Backend;Building+Scalable+Web+Applications" alt="typing" />
</p>

<p align="center">
  <b>Full Stack Developer | MERN Stack | Java Backend</b><br/>
  📍 Chennai, India &nbsp;|&nbsp; 📱 +91 9940659356 &nbsp;|&nbsp; 📧 <a href="mailto:tamilselvan.c155@gmail.com">Email</a>
  <br/>
  🌐 <a href="https://ts-profile.netlify.app/">Portfolio</a> &nbsp;|&nbsp; 💻 <a href="https://github.com/Tamilselvan155">GitHub</a> &nbsp;|&nbsp; 💼 <a href="https://www.linkedin.com/in/tamil-selvan-237237305">LinkedIn</a>
</p>

---

## 🧑‍💻 Professional Summary

Full Stack Developer with hands-on experience in building scalable web applications using the MERN stack and Java-based backend systems. Skilled in crafting responsive user interfaces, designing RESTful APIs, and optimizing application performance. Experienced across the full SDLC with a strong focus on clean architecture and maintainable code.

---

## ⚙️ Tech Stack

<p>
  <b>Frontend:</b> HTML5, CSS3, Bootstrap, Tailwind CSS, JavaScript, React.js, Next.js<br/>
  <b>Backend:</b> Java (Spring Boot), Node.js, Express.js<br/>
  <b>Databases:</b> MongoDB, MySQL, PostgreSQL, SQLite<br/>
  <b>Tools:</b> Git, GitHub, REST APIs, JWT, Electron, CI/CD Basics
</p>

---

## 💼 Experience

**Full Stack Developer**

* Designed and delivered scalable features from requirements to deployment
* Built and integrated frontend (React/Next) with backend services (Node/Java)
* Modeled databases and optimized queries for performance and reliability
* Debugged, tested, and resolved issues to ensure production stability
* Collaborated with cross-functional teams and maintained clean workflows with Git

---

## 🚀 Projects

### 📡 Live Vehicle Tracking & Fleet Analytics (SaaS)

* Real-time dashboards for vehicle monitoring using React.js
* Backend services with Java (Spring Boot) for tracking & reporting
* SaaS licensing and subscription-based access control
* GPS data processing for live tracking and analytics
* Robust REST APIs for frontend-backend communication

**Tech:** React.js · Spring Boot · PostgreSQL · REST · JWT

---

### 💻 Offline Gold Billing & Inventory System

* Cross-platform desktop app using Electron + MERN
* Modules: Billing, Inventory, Invoice, Gold Exchange, Admin
* Role-based access control and secure authentication
* Offline storage with SQLite and Excel backups

**Tech:** React.js · Node.js · Express · Electron · SQLite

---

### 🛒 E-Commerce Platform

* Full-stack app with user/admin dashboards
* REST APIs with MongoDB for products, users, orders
* JWT authentication and role management
* Responsive UI with React.js & Next.js

**Tech:** Next.js · React · Node.js · MongoDB · Tailwind

---

## 📌 Highlights

* End-to-end delivery (UI → API → DB → Deploy)
* Strong API design and integration skills
* Focus on performance, testing, and reliability

---

<p align="center">
  ⭐ If you like my work, consider following or reaching out!
</p>
